...
int main(){
    ...
    for(int i = 0; i < haystack.size(); i++){
        ...
        if(i>=needle.size()-1) haystackArr[haystack[i-needle.size()]-'a']--;
        if(i>= needle.size()-1 && memcmp(needleArr, haystackArr, sizeof(needleArr))==0){
            ans.insert({(hsh1[i+1] - hsh1[i+1-needle.size()]*pw1[needle.size()])%mod, (hsh2[i+1] - hsh2[i+1-needle.size()]*pw2[needle.size()])%mod});
        }
    }
    cout << ans.size();
}